  ...
  function filterTable() {
    ...
    Object.entries(filters).forEach((val) => {
      if ('date'===val[0]) {
        ...
        filteredData = filteredData.filter(row => row.state === val[1]);
      } else if ('country'===val[0]) {
        filteredData = filteredData.filter(row => row.country === val[1]);
      } else {
        filteredData = filteredData.filter(row => row.shape === val[1]);
      }
